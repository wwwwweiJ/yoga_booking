...
/// Register request. The studio is identified by its public token (from the
/// `/register/<token>` link), never a numeric id — so a member can only join a
/// studio whose link they were given, not by guessing.
#[derive(Debug, Deserialize, Serialize)]
...
async fn register(
    ...
    // Resolve the studio from its public token. A bad or unknown token can't
    // create an account (this is plain input validation, unlike the email path
    // below, which stays quiet to avoid enumeration).
